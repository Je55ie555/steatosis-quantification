...
def extract_areas_convert_to_um2(df):
    ...
    indexes = (
        []
    )  # returns the df index for each area (which subpatch the nucleus was in)
    mpps = df["mpps"].iloc[0]
    for index, row in df.iterrows():
        ...
        if len(areas) > 0:
            for nuc_area in areas:
                ...
                areas_in_um.append(
                    nuc_area * (mpps**2)
                )  # bc it is an area measurement, px*px
...
def combine_dataframes(subdf, nf):
    ...
    assert [
        i == True for i in nf["is_a_relevant_nucleus"].to_list()
    ]  # MAKE SURE nucdf ONLY takes in "relevant nuclei!
    ...
    for index, row in subdf.iterrows():  # each subpatch is a row
        ...
        assert (
            len(nucdf_slice["type"].to_list())
            == len(nucdf_slice["area_nuc(pxs)"].to_list())
        ) & (
            len(nucdf_slice["bbox_global(px)"].to_list())
            == len(nucdf_slice["centroid_global(px)"].to_list())
        )  # make sure same number of nuclei infos being added everywhere
...
def global_connected_component_anlysis(df, max_distance, centroids):
    ...
    print(
        f"         |_> completed clustring, found {len(components)} clusters [not accounting for number of nuclei/cluster]"
    )
...
def cluster_tree(tree, max_distance, centroids):
    neighbor_indices = tree.query_ball_tree(
        tree, r=max_distance
    )  ## Find all pairs of points whose distance is at most r
    G = (
        nx.Graph()
    )  # create a graph with the edges being the connection to each neighbor
    for i in range(len(neighbor_indices)):
        for j in neighbor_indices[i]:
            G.add_edge(i, j)
    components = list(
        nx.connected_components(G)
    )  ## list of indices that "belong" together
    num_components = nx.number_connected_components(G)
    clustered_nucs = (
        []
    )  # [ [(x,y), (x,y)], [(x,y)], ... ]. len list = total num of components,
    for component in components:
        ...
        for (
            ...
            component
        ):  # nuc is the index of the nucleus in respect to the original list
    ...
    (
        ...
        non_neigh_num,
        avg_dis_to_neighbor,
        avg_dis_to_nonneighbor,
        num_comm_neighs,
        degree,
        is_in_cluster,
    ) = graph_statistics(G, centroids, clustered_nucs)

    return clustered_nucs, [
        avg_clustering_coeffs,
        clustering_coeffs,
        num_neigh,
        non_neigh_num,
...
def graph_statistics(G, rel_nuclei_centroids, components):
    ...
    for n in neigh_indexes:
        ...
        for neigh in neighs:
            if not (node == neigh):
                avg_num_common_neighbors.append(
                    len([n for n in nx.common_neighbors(G, node, neigh)])
                )
...
def find_patchsize(df):
    ...
    coords = df[df["original_key"] == keys[0]][
        "global_x_coords(px)"
    ].to_list()  # has lots of subpatches
    return max(coords) - min(
        coords
    )  # was always process as a square, which is why this applies to y too


def run_CCA(base_path, max_distance, experiment_name, removed_df_label):
    """run CCA_analysis on imgs that have been processed by hovernet and algorithm"""
    processed = []

    for image_processed in os.listdir(base_path):
        name = image_processed
        path_to_fat_and_nuc_df = (
            f"{base_path}/{name}/dataframes/{name}_fat_and_nucs.pkl"
        )

        if not os.path.exists(path_to_fat_and_nuc_df):
            print(
                f"{name}_fat_and_nuc_df does not exist. Please make sure the path is correct."
            )
            assert 1 == 2
        else:
            ...
            if not (
                os.path.exists(
                    f"{base_path}{name}/nucleus_cluster_analysis/{experiment_name}/"
                )
            ):

                # make folder to save  exeriment information
                experiment_path = (
                    f"{base_path}{name}/nucleus_cluster_analysis/{experiment_name}/"
                )

                clusters_loc = experiment_path
                CCA_df_loc = experiment_path

                Path(f"{experiment_path}").mkdir(parents=True, exist_ok=False)

                processed.append(name)
                print("working on img:", name)

                algo_hvnet_df = pd.read_pickle(f"{path_to_fat_and_nuc_df}")
                print("---> loaded hovernet_and_algo_df")
                fat_loc = f"{base_path}{name}/saved_patches/fat_patches/"

                max_distance_px = int(
                    max_distance / algo_hvnet_df["mpps"].iloc[0]
                )  ## convert from_um to px
                ...
                centroids = extract_centroids(
                    df=algo_hvnet_df
                )  # globally cluster nuclei!

                components, tree, graph_stats = global_connected_component_anlysis(
                    df=algo_hvnet_df, max_distance=max_distance_px, centroids=centroids
                )

                print("finished finding components")
                with open(
                    f"{clusters_loc}{name}_{experiment_name}_clusters.pkl", "wb"
                ) as j:
                    pickle.dump(components, j)

                with open(
                    f"{clusters_loc}{name}_{experiment_name}_graph_stats.pkl", "wb"
                ) as j:
                    pickle.dump(graph_stats, j)

                cca_df, hull_df = process_components(
                    components=components,
                    algo_hvnet_df=algo_hvnet_df,
                    name=name,
                    path_to_fat_masks=fat_loc,
                    clusters_loc=clusters_loc,
                    data_saver_mode=False,
                    patch_size_px=patch_size_px,
                    removed_df_label=removed_df_label,
                    experiment_name=experiment_name,
                )
                print("saving final df")
                cca_df.to_pickle(f"{CCA_df_loc}{name}_{experiment_name}_global_CCA.pkl")
...
def process_components(
    ...
    for index, hrow in hull_df.iterrows():
        ...
        for original_key, patch_hulls in patch_data.items():
            ...
            if (
                ((subpatchx < bbox_xmax) & (subpatchxmax > bbox_x))
                & (subpatchy < bbox_ymax)
                & (subpatchymax > bbox_y)
            ):
        ...
        for patch_key in hull_patches:
            ...
            patch_hulls["patch_x"].append(
                patch_data[patch_key]["global_x_coords(px)"][0]
            )
            patch_hulls["patch_y"].append(
                patch_data[patch_key]["global_y_coords(px)"][0]
            )
    ...
    for original_key, patch_hulls in hulls_in_patch.items():
        if original_key >= 0:  # keep for testing
            patch, patch_x_global, patch_y_global = load_preprocessed_img(
                loc=path_to_fat_masks, wanted_key=original_key
            )
            ...
            hulls_mask = make_template(
                masks=masks,
                bboxs=bboxs,
                glob_x=patch_x_global,
                glob_y=patch_y_global,
                max_globx=patch_xmax,
                max_globy=patch_ymax,
                size=patch_size_px,
                canvas=np.ones_like(patch),
            )
            ...
            area_removed_by_subpatch = process_subpatches(
                remove, algo_hvnet_df["subpatch_size(px)"].iloc[0]
            )
...
def make_template(masks, bboxs, glob_x, glob_y, max_globx, max_globy, size, canvas):
    ...
    for i in range(len(masks)):
        ...
        if len(mask.shape) > 0:
            canvas[min_lcy:max_lcy, min_lcx:max_lcx] = np.invert(
                np.logical_and(
                    canvas[min_lcy:max_lcy, min_lcx:max_lcx],
                    mask[min_lmy:max_lmy, min_lmx:max_lmx],
                )
            )
    return canvas
...
def make_hulls(components):
    ...
    for cluster in components:
        ...
        if len(distinct_pts) > 2:
            ...
            mapped_verts = [
                cluster[v] for v in vert
            ]  # lst of global coords of vertices
            ...
            bbox, min_x, min_y, w, h = bounding_box(
                mapped_verts
            )  # min_x, min_y=top left corner of bb
            bboxs.append(bbox)
            verts_local = scaled_down(
                mapped_verts, min_x, min_y
            )  # scale to 0 to make mask
            mask = make_array_out_of_vertices(verts_local, width=w, height=h)
            masks.append(mask)

        data = {
            "global_hull_verts(px)": verts,
            "hull_area(pxs)": areas,
            "hull_masks": masks,
            "hull_bbox_global(px)": bboxs,
            "hull_centroids": hull_centroids_l,
        }
...
def overlay_masks(keys, loc, hull_df1, hull_df2, patch_size_px, algo_hvnet_df):
    ...
    for original_key in keys:
        patch, patch_x_global, patch_y_global = load_preprocessed_img(
            loc=loc, wanted_key=original_key
        )  # load the img
...
def cluster_by_group(
    ...
    for img in to_process:
        if (os.path.isdir(base_path + img)) & (
            os.path.exists(f"{base_path}{img}/dataframes/{img}_fat_and_nucs.pkl")
        ):
            ...
            algo_hvnet_df = pd.read_pickle(
                f"{base_path}{img}/dataframes/{img}_fat_and_nucs.pkl"
            )

            max_distance_1 = int(
                max_distance_1 / algo_hvnet_df["mpps"].iloc[0]
            )  ## convert from_um to px
            max_distance_2 = int(
                max_distance_2 / algo_hvnet_df["mpps"].iloc[0]
            )  ## convert from_um to px
            patch_size_px = int(
                patch_size_um / algo_hvnet_df["mpps"].iloc[0]
            )  ## convert from_um to px
            ...
            centroids_1, probabilities, bboxes, contours = extract_centroids_by_type(
                algo_hvnet_df, nuc_type_1, type_dic
            )
            centroids_2, probabilities, bboxes, contours = extract_centroids_by_type(
                algo_hvnet_df, nuc_type_2, type_dic
            )
            print(f"found {len(centroids_1)} nuclei of type {nuc_type_1}")
            print(f"found {len(centroids_2)} nuclei of type {nuc_type_2}")

            components_1, tree_1, graph_stats_1 = global_connected_component_anlysis(
                df=algo_hvnet_df, max_distance=max_distance_1, centroids=centroids_1
            )
            components_2, tree_2, graph_stats_2 = global_connected_component_anlysis(
                df=algo_hvnet_df, max_distance=max_distance_2, centroids=centroids_2
            )

            print("making hulls and adding data about overlapping patches")

            CCA_df1, hull_df1 = process_components(
                components_1,
                algo_hvnet_df,
                img,
                fat_loc,
                ...
                False,
                patch_size_px,
                removed_df_label=f"area_{nuc_type_1}_removed(px)",
            )
            CCA_df2, hull_df2 = process_components(
                components_2,
                algo_hvnet_df,
                img,
                fat_loc,
                ...
                False,
                patch_size_px,
                removed_df_label=f"area_{nuc_type_2}_removed(px)",
            )
            ...
            removed_area, patch_key_l = overlay_masks(
                keys, fat_loc, hull_df1, hull_df2, patch_size_px, algo_hvnet_df
            )
            CCA_df = add_data_to_df(
                algo_hvnet_df,
                patch_key_l,
                removed_area,
                label=f"total_area_removed(px)",
            )
            CCA_df[f"area_{nuc_type_1}_removed(px)"] = rm1
            CCA_df[f"area_{nuc_type_2}_removed(px)"] = rm2
            CCA_df[f"overlapping_{nuc_type_1}_{nuc_type_2}_area(px)"] = (
                CCA_df[f"area_{nuc_type_1}_removed(px)"]
                + CCA_df[f"area_{nuc_type_2}_removed(px)"]
            ) - CCA_df["total_area_removed(px)"]
            CCA_df.to_pickle(
                f"{base_path}{img}/dataframes/{img}_{experiment_name}_global_CCA.pkl"
            )
