...
def distance_to_blue(pts):
    ...
    dis = [
        math.sqrt((blue[0] - i[0]) ** 2 + (blue[1] - i[1]) ** 2 + (blue[2] - i[2]) ** 2)
        for i in pts
    ]
    return dis
...
def distance_to_white(pts):
    ...
    dis = [
        math.sqrt(
            (white[0] - i[0]) ** 2 + (white[1] - i[1]) ** 2 + (white[2] - i[2]) ** 2
        )
        for i in pts
    ]
    return dis
...
def find_avg_num_pix_where_blue_is_max(patch, mask):
    ...
    reds = patch_with_pixels_selected[
        :, 0
    ]  # high blue and red channels -> purple --> only pixels where blue is the "strongest" should be selected
...
def detect_CPA(
    ...
    for file in os.listdir(file_location):
        ...
        if (any([file in i for i in imgs_to_process]) & (file.endswith(file_type))) or (
            ...
            area_threshold_of_tissue_to_remove = (PATCH_SIZE * PATCH_SIZE * 0.05) / (
                mpp * 2
            )  # tissue object must take up at least 5% of area of patch
            ...
            global_threshold = skimage.filters.threshold_otsu(
                wsi_hsv_scanned_parts[:, 1]
            )
            ...
            for coords_y in range(start_y, finish_y, patch_size):
                for coords_x in range(start_x, finish_x, patch_size):
                    ...
                    if (
                        ...
                        tissue_mask_cleaned = np.invert(
                            morphology.remove_small_holes(
                                np.invert(tissue_mask),
                                area_threshold=area_threshold_of_tissue_to_remove,
                            )
                        )

                        liver_tissue_without_white_areas = np.sum(tissue_mask_cleaned)

                        percent_patch_with_liver_tissue = (
                            liver_tissue_without_white_areas / (patch_size**2)
                        ) * 100

                        if (
                            percent_patch_with_liver_tissue > 1
                        ):  # if more than 1% tissue was found
                            ...
                            xs_blue_ideal, ys_blue_ideal = convert_to_cartesian_space(
                                lst_of_degrees=[(-240)]
                            )

                            # red is located around 0 or 360
                            xs_red_ideal, ys_red_ideal = convert_to_cartesian_space(
                                lst_of_degrees=[(360)]
                            )
                            ...
                            hue_degrees = img_hsv[
                                tissue_mask_cleaned == True
                            ]  ### cluster only pixels in detected tissue
                            xs_detected_tissue, ys_detected_tissue = (
                                convert_to_cartesian_space(
                                    lst_of_degrees=hue_degrees[:, 0]
                                )
                            )
                            transformed_pixels_detected_tissue = np.column_stack(
                                [xs_detected_tissue, ys_detected_tissue]
                            )

                            # fit kmenas
                            kmeans = KMeans(
                                n_clusters=2, random_state=42, init=init, n_init=1
                            )
                            labels_original = kmeans.fit_predict(
                                transformed_pixels_detected_tissue
                            )
                            ...
                            hue_degrees_whole_patch = hsv_image[:, :, 0].reshape(
                                hsv_shape[0] * hsv_shape[1], 1
                            )

                            xs_whole_patch, ys_whole_patch = convert_to_cartesian_space(
                                lst_of_degrees=hue_degrees_whole_patch
                            )
                            transformed_pixels_whole_patch = np.column_stack(
                                [xs_whole_patch, ys_whole_patch]
                            )

                            labels = kmeans.predict(transformed_pixels_whole_patch)
                            labels = np.array(
                                [i + 1 for i in labels]
                            )  # we dont want a zero label :D for masking purposes
                            clustered_image = labels.reshape(
                                hsv_shape[0], hsv_shape[1]
                            )  # mask of the patch with labels
                            centers = kmeans.cluster_centers_

                            mask1 = np.logical_and(
                                (clustered_image == 1), tissue_mask_cleaned
                            )
                            mask2 = np.logical_and(
                                (clustered_image == 2), tissue_mask_cleaned
                            )
                            ...
                            xs_m1, ys_m1 = convert_to_cartesian_space(
                                lst_of_degrees=hue_mask1_degrees
                            )
                            xs_m2, ys_m2 = convert_to_cartesian_space(
                                lst_of_degrees=hue_mask2_degrees
                            )

                            # for each mask find the distance to blue and red
                            mask1_distance_to_blue = math.dist(
                                (xs_m1[0], ys_m1[0]),
                                (xs_blue_ideal[0], ys_blue_ideal[0]),
                            )
                            mask1_distance_to_red = math.dist(
                                (xs_m1[0], ys_m1[0]), (xs_red_ideal[0], ys_red_ideal[0])
                            )

                            mask2_distance_to_blue = math.dist(
                                (xs_m2[0], ys_m2[0]),
                                (xs_blue_ideal[0], ys_blue_ideal[0]),
                            )
                            mask2_distance_to_red = math.dist(
                                (xs_m2[0], ys_m2[0]), (xs_red_ideal[0], ys_red_ideal[0])
                            )
                            ...
                            red_distances = [
                                mask1_distance_to_red,
                                mask2_distance_to_red,
                            ]
                            blue_distances = [
                                mask1_distance_to_blue,
                                mask2_distance_to_blue,
                            ]

                            selected_red = (
                                np.argmin(red_distances) + 1
                            )  # b/c not zero index
                            selected_blue = (
                                np.argmin(blue_distances) + 1
                            )  # b/c not zero index
                            ...
                            selected_mask = np.logical_and(
                                (clustered_image == selected_blue), tissue_mask_cleaned
                            )
                            red_mask = np.logical_and(
                                (clustered_image == selected_red), tissue_mask_cleaned
                            )
                            ...
                            cpa_percent = (
                                num_blue_pixels / (tissue_pixels_no_fat + fat_tissues)
                            ) * 100
                            ...
                            total_tissue_pixels.append(
                                tissue_pixels_no_fat + fat_tissues
                            )
            ...
            data = {
                "name": name,
                "original_patch_key": key_l,
                "global_x_coords(pxs)": coordsxl,
                "global_y_coords(pxs)": coordsyl,
                "mpps": mpps,
                "num_blue_pixels": blue_pixels,
                "total_tissue_pixels(with_fat)": total_tissue_pixels,
                "num_pixels_tissue_without_fat": tissue_without_fat_pixels,
                "num_fat_pixels": fat_pixels,
            }
