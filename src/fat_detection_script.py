...
def create_csv(f, csv_files_path):
    ...
    csv_file.write(
        "WSI,global_threshold,patch_level,th_mpp,patch_key,patch_x,patch_y,Centroid_x,Centroid_y,Area,Bbox_x,Bbox_y,Bbox_width,Bbox_height,Extent,Axis_ratio,is_fat,Black_area,Circularity,Mpp,Magnification\n"
    )
    return csv_file
...
def open_mrxs(file_location, file, target):
    ...
    mpp_x = float(
        wsi.properties["mirax.LAYER_0_LEVEL_0_SECTION.MICROMETER_PER_PIXEL_X"]
    )
    mpp_y = float(
        wsi.properties["mirax.LAYER_0_LEVEL_0_SECTION.MICROMETER_PER_PIXEL_Y"]
    )
...
def open_czi(file_location, file, target):
    ...
    mpp = mpps_x * (
        10**6
    )  # mpps are in m => convert to micrometers to match mrxs units
    ...
    wsi_img_array = np.array(
        Image.fromarray(img_array).resize(
            (int(scanned_height // factor), int(scanned_width // factor))
        )
    )
    return (
        wsi_img_array,
...
def open_wsi(file_location, file, target):
    if file.endswith(".mrxs"):
        ...
        (
            ...
            mpp,
            wsi,
        ) = open_mrxs(file_location, file, target)
        img_type = "mrxs"
    ...
    if file.endswith(".czi"):
        ...
        (
            ...
            finish_y,
            mpp,
        ) = open_czi(file_location, file, target)
        img_type = "czi"
        wsi = np.nan
    return (
        wsi_img_array,
...
def convert_and_adjust(
    ...
    filled_bg_hole_size = convert_from_um_to_mpp(
        is_area=True, mpp=mpp, um=filled_bg_hole_size
    )
    filled_fg_hole_size = convert_from_um_to_mpp(
        is_area=True, mpp=mpp, um=filled_fg_hole_size
    )
    ...
    if mpp < orig_mpp:
        print(
            "READUJST MPP DISTANCE MEASUREMENTS, does not support <0.139 mpps at the moment"
        )  # should theoretically work, hasn't been tested yet. Saftey break :D
        assert 1 == 2
    else:
        factor = mpp / orig_mpp  # float
        int_factor = int(
            np.round(mpp / orig_mpp)
        )  ## not ideal..... .5> gets rounded up
...
def open_patch(
    img_type, wsi, wsi_img_array, coords_x, coords_y, patch_size, patch_level
):
    """opens a patch of the wsi, returns patch as an array"""

    if img_type == "mrxs":
        patch = np.array(
            wsi.read_region((coords_x, coords_y), patch_level, (patch_size, patch_size))
        )  # THE ARRAY IS STORED AS y,x
    if img_type == ".czi":
        patch = wsi_img_array[
            coords_x : coords_x + patch_size, coords_y : coords_y + patch_size
        ]
    return patch
...
def global_threshold(wsi, img_type):
    ...
    threshold = skimage.filters.threshold_otsu(
        wsi[scanned_sections == True][:, 0:3].std(axis=1)
    )  # threshold with variance information
    return threshold
...
def preprocess(
    patch, global_threshold, filled_bg_hole_size, filled_fg_hole_size, kernel
):
    """
    ...
    filled_patch = remove_holes_from_patch(
        mask, filled_fg_hole_size, filled_bg_hole_size
    )
...
def seed_watershed(patch, max_area, magnification):
    ...
    filled_patch = fill_large_holes(
        patch=patch, max_fat_area=max_area
    )  # makes sure no seeds are placed in background. Only objs that are potentially fat remain
    distance_map = skimage.filters.gaussian(
        ndi.distance_transform_edt(filled_patch), sigma=5 // magnification
    )
    peak_max_coords = peak_local_max(
        distance_map, min_distance=50 // magnification, threshold_abs=8 // magnification
    )  # returns coordinates of local peaks in img
    peak_mask = np.zeros(distance_map.shape, dtype=bool)  # create mask
    peak_mask[tuple(peak_max_coords.T)] = True  # fill in local maxes
    h, w, _ = find_maxima(
        distance_map, peak_mask, 10 // magnification
    )  # find the maxima of the local maxes.
    local_max_mask = np.zeros(distance_map.shape, dtype=bool)
    for m_x, m_y in zip(w, h):
        local_max_mask[m_y, m_x] = (
            True  # set local_max_mask to true at the coordinates of the maxima
        )
...
def watershed(closed_patch, max_area, mag, preprocessed_patch):
    ...
    filled_patch = fill_large_holes(
        patch=preprocessed_patch, max_fat_area=max_area
    )  # Only objs that are potentially fat remain
    distance_map = skimage.filters.gaussian(
        ndi.distance_transform_edt(filled_patch), sigma=5 // mag
    )
    seg = skimage.segmentation.watershed(
        -distance_map, seeds, mask=filled_patch, watershed_line=False
    )  # only points where mask == True are labeled.
...
def process_subpatches(
    ...
    for y in range(0, patch_size, size):
        for x in range(0, patch_size, size):
            ...
            foreground_area = np.sum(
                b_patch[scanned_subpatch == 1] == 0
            )  # I don't need to do the scanned==0 part, could delete.
...
def save_subpatch_info_into_df(
    subpatch_analysis, output_loc, name_for_file, patch_size
):
    ...
    data = {
        ...
        "subpatch_size(px)": subpatch_size,
        "subpatch_key": subpatch_key,
        "global_x_coords(px)": global_x_coords,
        "global_y_coords(px)": global_y_coords,
        "fat_area(px)": fa,
        "black_area(px)": ba,
        "mpps": mpps,
    }
...
def run_workflow(
    ...
    for file in os.listdir(file_location):
        ...
        if (any([file in i for i in imgs_to_process]) & (file.endswith(file_type))) or (
            (len(imgs_to_process) == 0) & (file.endswith(file_type))
        ):
            ...
            (
                ...
                img_type,
                wsi,
            ) = open_wsi(file_location, file, target)
            ...
            if run_hovernet:
                ...
                print(
                    f"             |_ location hovernet storage (if hovernet enabled):{hovernet_loc}"
                )
            ...
            patch_size = int(
                math.ceil(patch_size / SUBPATCH_SIZES_FACTOR) * SUBPATCH_SIZES_FACTOR
            )
            print(
                f"-------- patch_size adjusted for even fitting of subpatches: {patch_size}"
            )
            print(f"------- number of subpatches in a patch: {SUBPATCH_SIZES_FACTOR}")
            subpatch_sizes = [int(patch_size / SUBPATCH_SIZES_FACTOR)]

            print(
                "----------  mapping patch size to a suitable value for subpatch processing"
            )
            ...
            subpatch_analysis = []
            print(
                f"---------- total number of patches in wsi (at patch size {patch_size} (px)): { math.ceil(float(scanned_width / patch_size)) * math.ceil(float(scanned_height / patch_size))}"
            )
            ...
            for coords_y in range(start_y, finish_y, patch_size):
                for coords_x in range(start_x, finish_x, patch_size):
                    ...
                    if (
                        key >= 0
                    ):  # keep in case a particular patch key is of interest :)
                        ...
                        scanned_patch = return_scanned_parts(
                            img=patch, img_type=img_type
                        )
                        closed_patch, preprocessed_patch, gray_patch = preprocess(
                            patch=patch,
                        ...
                        preprocessed_patch[scanned_patch == 0] = (
                            1  # fill in unscanned regions as background
                        )

                        if (
                            forground_area > 0
                        ):  # only patches with tissue on it will be processed further

                            save_patches(
                                preprocessed_patch=preprocessed_patch,
                                patch_with_fat_objs=patch_with_fat_objects,
                                binary_loc=binary_loc,
                                fat_loc=fat_loc,
                                key=key,
                                coords_x=coords_x,
                                coords_y=coords_y,
                            )  # patches need to be saved for connected component anlysis (fat overlap) and for nuclei anaysis (check if nuclei are located in an area with tissue)

                            if run_hovernet:
                                if (
                                    patches_stored % 10 == 0
                                ):  # change to mod 10 after testing.
                                    stored_patches.append(
                                        [patch, key]
                                    )  # stored_patches.append([Image.fromarray(patch), key])
                                    start_hovernet(
                                        stored_patches=stored_patches,
                                        save_loc=hovernet_loc,
                                        hovernet_command=hovernet_command,
                                        master_loc=hovernet_master_loc,
                                        img_name=f,
                                        coords_x=coords_x,
                                        coords_y=coords_y,
                            ...
                            if len(subpatch_sizes) > 0:
                                ...
                                for size in subpatch_sizes:
                                    ...
                                    subpatch_analysis, ba, fa = process_subpatches(
                                        preprocessed_patch=preprocessed_patch,
                                        size=size,
                                        patch_size=patch_size,
                                        subpatch_analysis=subpatch_analysis,
                                        scanned=scanned_patch,
                                        patch_with_fat=patch_with_fat_objects,
                                        coords_x=coords_x,
                                        coords_y=coords_y,
                                        key=key,
                                        mpp=mpp,
                                    ...
                                    total_fat_area += fa
                                assert (total_foreground_area == forground_area) & (
                                    total_fat_area
                                    == np.count_nonzero(patch_with_fat_objects == 0)
                                )
                                subpatch_df = save_subpatch_info_into_df(
                                    subpatch_analysis,
                                    output_loc,
                                    file.replace(img_type, ""),
                                    patch_size,
                                )
            ...
            if run_hovernet:
                ...
                if (
                    patches_stored > 0
                ):  ## run hovernet for the last of the pictures in the stored_patch_que
                    start_hovernet(
                        stored_patches=stored_patches,
                        save_loc=hovernet_loc,
                        hovernet_command=hovernet_command,
                ...
                consolidated_hovernet_df = crawl_through_data(
                    hovernet_loc
                )  # CAVE contains nuclei in unscanned regions!
                assert (
                    len(consolidated_hovernet_df) > 0
                )  # makes sure that nuclei were detected
                raw_hovernet_df = globalize_hovernet_data(
                    consolidated_hovernet_df, binary_loc
                )
                relevant_nuclei_df = raw_hovernet_df[
                    raw_hovernet_df["is_a_relevant_nucleus"] == True
                ]
                ...
                algo_hvnet_df = combine_dataframes(
                    subdf=subpatch_df, nucdf=relevant_nuclei_df
                )  # lists all the nuclei (with info) in each subpatch
                algo_hvnet_df["patch_size(px)"] = [patch_size] * len(algo_hvnet_df)
                algo_hvnet_df["subpatch_size(px)"] = [subpatch_sizes[0]] * len(
                    algo_hvnet_df
                )
