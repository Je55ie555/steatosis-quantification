...
parser = argparse.ArgumentParser(prog='Fat and Nuclei Detection',
    description='This code processes a WSI in patches in order to detect fatty vesicles. Optionally, if analyzing a H&E WSI, nuclei can be detected using [HoVer-Net](https://github.com/vqdang/hover_net).'
    epilog='See ReadMe for more information')

parser.add_argument('-i', '--input-dir', required=True, help="path to WSI image data directory") #file_loc
...
parser.add_argument('--WSI-queue', required=True, help="to process all files in input dir, WSI-queue = []. Alternatively, can be a list of the file names of the WSIs to be processed in the input directory. Default = []", type=list, default=[]) #imgs_to_process
...
parser.add_argument( '--data-saver-mode', required=False, help="handle HoVer-Net temporary data", type=bool, default=False )
...
parser.add_argument( '--min-extent', required=False, help="minimum extent of a fat vesicle. Default is 0.5", type=int, default=0.5) 
parser.add_argument( '--max-axis-ratio', required=False, help="maximum axis ratio of a fat vesivle. Default: 2", type=int, default=2) 
parser.add_argument( '--subpatch-sizes', required=False, help="patch size of subpatches (in µm). List of ints. Default = [70]", type=list, default=[70]) 
...
run_time = datetime.now().striftime("%Y%m%d-%H%M%S")
...
if args.run_hovernet:
    ...
    if [arg is None for arg in hovernet_args]:
        ...
        hovernet_command = [args.hovernet_gpu, args.num_nuc_types, args.hovernet_model_mode, args.hovernet_weights_file, args.hovernet_run_function]
    ...
    KERNEL= args.disk,
    SUBPATCH_SIZES = args.subpatch_sizes,
    ...
    imgs_to_process = args.WSI_queue
)