...
parser.add_argument(
    "--input-dir", required=True, help="path to WSI image data directory"
)  # file_loc
...
parser.add_argument(
    "--run-hovernet",
    required=False,
    help="must be 'True' to run HoVer-Net",
    action="store_true",
)
parser.add_argument("--hovernet-dir", required=False, help="path to HoVer-Net code")
parser.add_argument(
    "--hovernet-weights-file",
    required=False,
    help="path to weights (.tar file)",
    type=str,
)
parser.add_argument(
    "--hovernet-gpu",
    required=False,
    help="which gpu number to use run HoVer-Net",
    type=str,
)
parser.add_argument(
    "--hovernet-num-nuc-types",
    required=False,
    help="how many nuclei types are to be detected. Depends on weights being used.",
    type=str,
)
parser.add_argument(
    "--hovernet-batch-size", required=False, help="what batch size to use", type=str
)
parser.add_argument(
    "--hovernet-model-mode",
    required=False,
    help="can either be 'fast' or 'original'",
    type=str,
)
parser.add_argument(
    "--hovernet-run-function",
    required=False,
    help="see HoVer-Net documentation. Default: './run_infer.py'",
    type=str,
...
parser.add_argument(
    "--data-saver-mode",
    required=False,
    help="handle HoVer-Net temporary data",
    action="store_true",
)
...
parser.add_argument(
    "--subpatch-size-factor",
    required=False,
    help=" int representing how many subpatches a patch will be divided into. Default=20.",
    type=int,
    default=20,
)
parser.add_argument(
...
if args.run_hovernet:
    ...
    for arg in hovernet_args:
        if arg is None:
            print(
                "if running HoVer-net, you must provide all the HoVer-net parameters:"
            )
            print("     --hovernet-dir")
