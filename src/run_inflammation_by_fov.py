...
parser = argparse.ArgumentParser(prog='Process nuclei by Field of View',
    description='This script allows an analysis of the number of nuclei clusters in a given (square-shaped) field of view (FOV).'
    epilog='See ReadMe (Additional Nucleus Analysis) for more information')
...
parser.add_argument( '--output-dir', required=True, help="path to output directory", type=str)
...
processed = os.listdir(f"{save_loc}/{experiment_name}/num_foki_per_20fov/") #prevent images from being rerun :D. Remember to change hardcoded part if using another fov.
processed = [i.replace("_foki_per_20xfov.pkl", "") for i in processed] # creates a list of the names of files that have already been processed.

run_Infl_by_foci_for_all_imgs(path=path,
                            processed = processed,
